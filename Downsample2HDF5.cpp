...
void write_header_attributes_in_hdf5(hid_t handle, gadget_header& header, int64_t stride)
{
  /*Write a header. Note handle should be a file handle.*/
  H5Gcreate(handle,"Header",H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  int npart_thisfile[N_TYPE];
  for(int i = 0; i < N_TYPE; i++)
    npart_thisfile[i] = header.npart[i]/stride;

  unsigned int npart_total[N_TYPE];
  for(int i = 0; i < N_TYPE; i++)
    npart_total[i] = header.npartTotal[i]/stride;

  H5LTset_attribute_int(handle,"Header","NumPart_ThisFile",(int *)npart_thisfile,N_TYPE);
  H5LTset_attribute_uint(handle,"Header","NumPart_Total",(unsigned int *)npart_total,N_TYPE);
...
int main(int argc, char* argv[]){
     ...
     /*Convert the header*/
     head=snap.GetHeader();
     write_header_attributes_in_hdf5(hdf_file, head, stride);
     ...
     for(int i = 0; i < N_TYPE; i++){
         ...
         for(it=blocks.begin() ; it != blocks.end(); it++){
             int rank = 1;
             hsize_t size[2] = {snap.GetBlockSize(*it,i,stride)/sizeof(float)};
             if(*it == "POS " || *it == "VEL "){
                 rank = 2;
                 size[0]/=3;
                 size[1]=3;
             ...
             if(*it == "ID  "){
	         size[0] = snap.GetBlockSize(*it,i,stride)/sizeof(int64_t);
                 int64_t * blockdata = (int64_t *)malloc(snap.GetBlockSize(*it,i,stride));
                 snap.GetBlock(*it,(void *)blockdata,snap.GetNpart(i),0,0,stride);
   	        ...
   	        float * blockdata =(float *) malloc(sizeof(float)*snap.GetBlockSize(*it,i,stride));
                snap.GetBlock(*it,(void *)blockdata,snap.GetNpart(i),0,0,stride);
